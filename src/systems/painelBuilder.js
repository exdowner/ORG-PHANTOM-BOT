...
module.exports = function (config, lista1 = [], lista2 = []) {
    ...
    const embed = new EmbedBuilder()
        .setColor("#2b2d31")
        .setTitle(`${config.modo || "Mobile"} | ${config.valor || "3,00"}`)
        // Coloque aqui a URL da logo do fantasma
        // .setThumbnail("https://LINK_DA_LOGO.png")
        .addFields(
            {
                name: config.modoMisto
                    ? `📱 1 Emulador (${lista1.length}/${qtdMax})`
                    : `🧊 Gel Normal (${lista1.length}/${qtdMax})`,
                value:
                    lista1.length > 0
                        ? lista1.map(j => `<@${j.id}>`).join("\n")
                        : "Vazio",
                inline: false
            },
            {
                name: config.modoMisto
                    ? `💻 2 Emuladores (${lista2.length}/${qtdMax})`
                    : `♾️ Gel Infinito (${lista2.length}/${qtdMax})`,
                value:
                    lista2.length > 0
                        ? lista2.map(j => `<@${j.id}>`).join("\n")
                        : "Vazio",
                inline: false
            }
        )
